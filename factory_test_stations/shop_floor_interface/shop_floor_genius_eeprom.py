    ...
    def save_results(self, uut_sn, log_items):
        ...
        for log_item in log_items[1:]:
            ...
            for it in [k for k, d in self._mes_dic.items() if d == name]:
                log_2_mes[it] = value
                if res == 'FAIL' and int(idx) > 0:
                    err_codes.append(idx)

        # 日期格式转换-用于数据上传
        log_2_mes['START_TIME'] = datetime.datetime.strptime(log_2_mes['START_TIME'], '%Y%m%dT-%H%M%SZ')
        log_2_mes['END_TIME'] = datetime.datetime.strptime(log_2_mes['END_TIME'], '%Y%m%dT-%H%M%SZ')
    ...
    def p5385_in_lot_info(self, uut_sn):
        ...
        while count < self._max_retries and (not success):
            try:
                ...
                logging.info(f'Fail to call P5385_in_lot. Ex: {str(e)}')
            count += 1
    ...
    def lcd_avi_myzy_log_loader(self, lcd_avi_data):
        ...
        while count < self._max_retries and (not success):
            try:
                ...
                logging.info(f'Fail to call lcd_avi_myzy_log_loader. Ex:{str(e)}')
            count += 1
    ...
    def gseo_mail_ws(self, subject, content):
        ...
        while count < self._max_retries and (not success):
            try:
                ...
                logging.info(f'Fail to call sendMail. retries = {count+1}, Ex: {str(e)}')
            count += 1
        return res
    # </editor-fold>
...
def daemon_upload(*add):
    ...
    while HostProgressName in [psutil.Process(c).name() for c in psutil.pids()]:
        file_to_upload = None
        ...
        if ((file_to_upload is not None)
            ...
            try:
                ...
                logging.info(f'Fail to upload data {MES_RECV_MAIL_LIST}, err: {str(e)}')
        ind += 1
            ...
            if hasattr(validate_res, 'STATUS'):
                ...
                if validate_res.STATUS.upper() == 'SUCCESS':
                    ...
                    if validate_res.INSPECTION_RESULT in ['PASS']:
                        ...
                        msg = '过站检测失败 站点状态: {0}, 目检结果: {1}'.format(validate_res.STATUS, inspection)
                else:
