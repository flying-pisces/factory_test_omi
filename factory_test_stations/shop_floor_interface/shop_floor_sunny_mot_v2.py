        ...
        while count < self._max_retries and (not success):
            try:
                ...
                if resp.status_code == requests.codes.ok:
                    ...
                    logging.info(f"Success to validate. Result: {res['Result']} TestTimes : {res['TestTimes']}")
                    success = True
            except:
                pass
            count += 1
...
def initialize(station_config):
    ...
    log_dir = os.path.join(os.path.curdir, '../shop_floor_interface/log')
...
if __name__ == '__main__':
    ...
    logging.info('----------------------------')
    logging.debug('+' * 20)
    serial_number = 'T1234567891234'
    logging.info('----------------------------{0}'.format(serial_number))
    if ok_to_test(serial_number):
        pass
        save_results_from_logs(r'C:\oculus\factory_test_omi\factory_test_stations\factory-test_logs\T1234567891234_seacliff_mot-03_20210817-094747_P.log')
    else:
