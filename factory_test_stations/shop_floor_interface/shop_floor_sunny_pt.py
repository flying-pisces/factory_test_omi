    ...
    def validate_AAB(self, uut_sn):
        ...
        while count < self._max_retries and (not success):
            try:
                ...
                logging.error(f'---> fail to validate aab. Exp: {str(e)}')
            count += 1
        if not success:
            logging.error(f'fail to validate aab. {json.dumps(arg)}')
            logging.info(f'<--- {resp}')
        return res
    ...
    def insert_data(self, station_name, arg):
        ...
        if station_name_dic is not None:
            while count < self._max_retries and (not success):
                ...
                count += 1
        if not success:
            logging.info('---> fail to  validate aab : {0}'.format(json.dumps(arg)))
            logging.info(f'<--- {resp}')
        return res
    # </editor-fold>
...
MACHINE_TYPE = 'PNL'
STATION_ID = 'seacliff_paneltesting-02'
MAC_ID_FILER = '以太网'
