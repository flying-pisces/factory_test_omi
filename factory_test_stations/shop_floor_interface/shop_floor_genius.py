...
SF_URL = 'http://192.168.100.96:8012/InLotWS/Service/P5385InLotWS.asmx'
SF_LOADER_URL = 'http://192.168.100.96:8012/LoaderWS/Service/SummaryLoader.asmx'

SF_ACTIONS = {
    'GetInLotInfo': 'GetInLotInfo',
    'SummaryLoaderGet': 'SummaryLoaderGet',
}
...
class ShopFloor_genius(object):
    ...
    def get_test_results(self, uut_sn):
        ...
        resp = requests.post(SF_URL + SF_ACTIONS['GetInLotInfo'], timeout=self._time_out,
        ...
        if resp.status_code == requests.codes.ok:
            ...
            results = root.findall('GetInLotInfoResponse/GetInLotInfoResult', root.nsmap)
            result_keys = ['STATUS', 'LOT', 'STATION', 'PART_NO', 'OPERATION_STATUS']
    ...
    #                          headers=self._headers,
    #                          data=json.dumps(arg))
    #     logging.debug('<--- {0}'.format(json.dumps(resp.text)))
    #     if resp.status_code == requests.codes.ok:
    ...
    #         logging.debug('---> validate aab : {0}'.format(len(arg)))
...
def daemon_upload(*add):
    logging.info('启动后台数据上传的任务')
    for ind in range(1, 5):
        print('*-- %d \n'%ind)
        time.sleep(2)
    logging.info('停止后台数据上传任务')
...
if not logger.hasHandlers():
    ...
    _ex_shop_floor = ShopFloor_genius(MACHINE_TYPE, STATION_ID)  # type: ShopFloor_sunny
