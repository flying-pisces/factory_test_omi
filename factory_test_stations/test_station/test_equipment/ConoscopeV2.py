...
CONOSCOPE_LIB_NAME = "ConoscopeLib_TaprisiotRetrofit.dll"
...
class Conoscope:
    ...
    class ComposeOutputImage(Enum):
        ComposeOutputImage_XYZ = 0
        ComposeOutputImage_X = 1
        ComposeOutputImage_Y = 2
        ComposeOutputImage_Z = 3
        ComposeOutputImage_Yxz = 4
        ComposeOutputImage_None = 5

    class SpectroCalibrationType(Enum):
        SynthetizedBias = 0
        TrueBlack = 1
        Disable = 2

    class SpectroBiasMeasurement(Enum):
        CaptureOnly = 0
        CaptureForced = 1
        UseStored = 2

    class SpectroSequenceState(Enum):
        SpectroSequenceState_Idle = 0
        SpectroSequenceState_Processing = 1
        SpectroSequenceState_MeasurementDone = 2
        SpectroSequenceState_Error = 3
    ...
    class ConoscopeDebugSettings(ctypes.Structure):
        _fields_ = [
            ...
            ("emulatedSpectrometer", ctypes.c_bool),
            ("dummySpectroMeasPath", ctypes.c_char_p)]
    ...
    class CaptureSequenceConfig(ctypes.Structure):
        _fields_ = [
            ...
            ("bSpectro", ctypes.c_bool),
            ("bApplyBias", ctypes.c_bool),
            ("bApplyFlatField", ctypes.c_bool),
            ("bApplyDeconvolution", ctypes.c_bool),
            ("bColorimetricCorrectionEnable", ctypes.c_bool),
            ("bLuminanceCorrectionEnable", ctypes.c_bool)]
    ...
    class SpectroConfig(ctypes.Structure):
        _fields_ = [
            ("integrationTimeUs", ctypes.c_int),
            ("nbMeasurement", ctypes.c_int),
            ("bAuto", ctypes.c_bool),
            ("bSave", ctypes.c_bool)]

    class SpectroData(ctypes.Structure):
        _fields_ = [
            ("waveLength", ctypes.c_float),
            ("value", ctypes.c_float)]

    class SpectroMeasurement(ctypes.Structure):
        _fields_ = [
            ("integrationTimeUs", ctypes.c_int),
            ("nbMeasurement", ctypes.c_int)]
            # std::vector < SpectroData_t > data;

    class SpectroSequenceConfig(ctypes.Structure):
        _fields_ = [
            ("integrationTimeUs", ctypes.c_int),
            ("nbMeasurement", ctypes.c_int),
            ("bAuto", ctypes.c_bool),
            ("eCalibrationType", ctypes.c_int),
            ("eBiasMeasurement", ctypes.c_int),
            ("bApplyBias", ctypes.c_bool),
            ("bApplyFlatField", ctypes.c_bool),
            ("bApplyDeconvolution", ctypes.c_bool),
            ("bSave", ctypes.c_bool)]

    class SpectroSequenceStatus(ctypes.Structure):
        _fields_ = [
            ("eState", ctypes.c_int),
            ("lastMeasurement", ctypes.c_int)]
    ...
    def __init__(self, *, emulate_camera=False, emulate_wheel=False, emulate_spectro=False):
        ...
        # default value (spectro disable and all options enable)
        captureSequenceConfig = {
            "eOuputImage": Conoscope.ComposeOutputImage.ComposeOutputImage_XYZ.value,
            "bSpectro": False,
            "bApplyBias": True,
            "bApplyFlatField": True,
            "bApplyDeconvolution": True,
            "bColorimetricCorrectionEnable": True,
            "bLuminanceCorrectionEnable": True
        }
        Conoscope.__FillStructure(self.captureSequenceConfig, captureSequenceConfig)

        self.captureSequenceStatus = Conoscope.CaptureSequenceStatus()

        self.measureAEStatus = Conoscope.MeasureAEStatus()

        self.spectroConfig = Conoscope.SpectroConfig(10000, 1, False, True)
        # set to true black (1)
        #self.spectroSequenceConfig = Conoscope.SpectroSequenceConfig(10000, 1, False,
        #                                                             1, 1, True, True, True, True)
        self.spectroSequenceConfig = Conoscope.SpectroSequenceConfig()

        spectroSequenceConfig = {"nbMeasurement": 1,
                                 "bAuto": False,
                                 "eCalibrationType": Conoscope.SpectroCalibrationType.TrueBlack.value,
                                 "eBiasMeasurement": Conoscope.SpectroBiasMeasurement.UseStored.value,
                                 "integrationTimeUs": 10000,
                                 "bApplyBias": True,
                                 "bApplyFlatField": True,
                                 "bApplyDeconvolution": True,
                                 "bSave": True}
        Conoscope.__FillStructure(self.spectroSequenceConfig, spectroSequenceConfig)

        dllPath = os.path.join(os.path.join(os.getcwd(), Conoscope.DLL_PATH, CONOSCOPE_LIB_NAME))  # ugly hack
        ...
        CmdSpectroProto = ctypes.WINFUNCTYPE(
            ctypes.c_char_p,  # Return type.
            ctypes.c_void_p)
        self.__CmdSpectro = CmdSpectroProto(("CmdSpectro", conoscopeDll))

        CmdSpectroSequenceProto = ctypes.WINFUNCTYPE(
            ctypes.c_char_p,  # Return type.
            ctypes.c_void_p)
        self.__CmdSpectroSequence = CmdSpectroSequenceProto(("CmdSpectroSequence", conoscopeDll))
        ...
        ret = self.CmdSetDebugConfig({"debugMode": False,
                                      ...
                                      "emulatedSpectrometer": emulate_spectro})
        print("instance done")
    ...
    def CmdSpectro(self, config):
        Conoscope.__FillStructure(self.spectroConfig, config)
        return Conoscope.__Result(self.__CmdSpectro(ctypes.byref(self.spectroConfig)))

    def CmdSpectroSequence(self, config):
        Conoscope.__FillStructure(self.spectroSequenceConfig, config)
        return Conoscope.__Result(self.__CmdSpectroSequence(ctypes.byref(self.spectroSequenceConfig)))
