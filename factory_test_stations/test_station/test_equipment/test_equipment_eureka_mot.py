...
class EurekaMotEquipmentError(Exception):
    pass


class EurekaMotEquipment(hardware_station_common.test_station.test_equipment.TestEquipment):
    """
    ...
    def do_measure_and_export(self, pos_name, pattern_name):
        ...
        if setup_cfg is None:
            raise EurekaMotEquipmentError('Fail to set setup_config.')
        out_image_mode = 0
        ...
        if isinstance(exposure_cfg, str) or isinstance(exposure_cfg, tuple):
            if self._station_config.SEQ_CAP_INIT_CONFIG['bUseExpoFile']:
                ...
                if not os.path.exists(capture_seq_exposure):
                    raise EurekaMotEquipmentError('Fail to Find config for {0}.'.format(capture_seq_exposure))
            ...
            if int(processState.value) not in [0, 6]:  # CaptureSequenceState_NotStarted , CaptureSequenceState_Done
                raise EurekaMotEquipmentError('Fail to CmdMeasureSequence.{0}'.format(processState))
            ...
            if ret['Error'] != 0:
                EurekaMotEquipmentError('Fail to CmdCaptureSequence.')
            self.__check_seq_finish()
            del captureSequenceConfig
        else:
            raise EurekaMotEquipmentError(f'Not support for this config. {exposure_cfg}')
    ...
    def initialize(self):
        ...
        try:
            ...
            raise EurekaMotEquipmentError(f'Except: {str(e)}, PATH: {conoscope_pth}')
...
class MotAlgorithmHelper(object):
    ...
    def get_export_data(cls, filename, station_config=None):
        ...
        if not dtype:
            raise EurekaMotEquipmentError('unsupported bin file. {0}'.format(filename))
    ...
    def calc_gl_for_brightdot(W255_stats_summary, WRGBBoresight_stats_summary, module_temp=30):
        ...
        Y_r = WRGBBoresight_stats_summary["R_Lum"] * (1 + (ModuleTemp - Temp_R) * dLum_R)
        Y_g = WRGBBoresight_stats_summary["G_Lum"] * (1 + (ModuleTemp - Temp_G) * dLum_G)
        Y_b = WRGBBoresight_stats_summary["B_Lum"] * (1 + (ModuleTemp - Temp_B) * dLum_B)

        x_r = WRGBBoresight_stats_summary["R_x"] + (ModuleTemp - Temp_R) * dColor_x_R
        y_r = WRGBBoresight_stats_summary["R_y"] + (ModuleTemp - Temp_R) * dColor_y_R
        x_g = WRGBBoresight_stats_summary["G_x"] + (ModuleTemp - Temp_G) * dColor_x_G
        y_g = WRGBBoresight_stats_summary["G_y"] + (ModuleTemp - Temp_G) * dColor_y_G
        x_b = WRGBBoresight_stats_summary["B_x"] + (ModuleTemp - Temp_B) * dColor_x_B
        y_b = WRGBBoresight_stats_summary["B_y"] + (ModuleTemp - Temp_B) * dColor_y_B
    ...
    @staticmethod
    def calc_gl_for_brightdot_v2(W255_stats_summary,
                              R255_stats_summary,
                              G255_stats_summary,
                              B255_stats_summary,
                              RGBBoresight_stats_summary,
                              module_temp=30):
        # % load temperature of whitedot pattern
        ModuleTemp = module_temp
        # % set color drift coefficients
    ...
    def rgbboresight_parametric_export(self, module_temp=30, xfilename=r'rgbboresight_x_float.bin'):
        ...
        for stat in [c for c in statsB
                     if c['MajorAxisLength'] >= Length_thresh ]:
...
if __name__ == '__main__':
    ...
    raw_data_dir = r"C:\ShareData\Oculus_RawData\005L_eureka_mot-0001_20220621-182943"
    ...
    gl_whitedot = aa.calc_gl_for_brightdot_v2(w255_result, r255_result, g255_result, b255_result, boresight_result, module_temp=m_temp['WhiteDot'])
