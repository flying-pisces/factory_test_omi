...
class EurekaMotFixture(hardware_station_common.test_station.test_fixture.TestFixture):
    ...
    def initialize(self, **kwargs):
        ...
        if self._station_config.FIXTURE_PARTICLE_COUNTER:
            ...
            if not self._particle_counter_client.connect():
                raise EurekaMotFixtureError(f'Unable to open particle counter port: {kwargs}')
    ...
    def close(self):
        try:
            self._operator_interface.print_to_console("Closing EurekaMOT Fixture\n")
    ...
    def mov_abs_xy_wrt_alignment(self, x, y):
        ...
        if self._alignment_pos is None:
            raise EurekaMotFixtureError('alignment command should be executed firstly.')
    ...
    def mov_abs_xya(self, x, y, a):
        ...
        if math.fabs(y) < self._y_limit_pos and not math.isclose(a, 0):
            ...
            raise EurekaMotFixtureError(msg)
        if a > math.fabs(self._a_limit_pos):
            raise EurekaMotFixtureError('fail to move abs_xya.  | a | = {} < {} '.format(a, self._a_limit_pos))
    ...
    def query_temp(self, parts=QueryTempParts.EquipmentAmbient):
        ...
        if not (temp_l <= float(deters[1]) <= temp_h):
            raise EurekaMotFixtureError(f'temperature {deters[1]} is over range [{temp_l}, {temp_h}] does not make sense.')
    ...
    def _parse_response(self, regex, resp):
        ...
        if len([c for c in resp if c.find(self._error_msg) >= 0]) > 0:
            raise EurekaMotFixtureError('error msg from fixture. %s' % resp)
        if not resp:
            raise EurekaMotFixtureError('unable to get data from fixture.')
        items = list(filter(lambda r: re.search(regex, r, re.I | re.S), resp))
        if not items:
            raise EurekaMotFixtureError('unable to parse msg. {}'.format(resp))
        ...
        if self._particle_counter_client is not None:
            ...
            while (retries <= 10) and (val is None):
                try:
                    ...
                    retries += 1
            if val is None:
                raise EurekaMotFixtureError('Failed to start particle counter .')
        ...
        if self._particle_counter_client is not None:
            ...
            while (retries <= 10) and (val is None):
                try:
                    ...
                    retries += 1
            if val is None:
                raise EurekaMotFixtureError('Failed to read data from particle counter.')
            return val
        ...
        if self._particle_counter_client is not None:
            ...
            while (retries <= 10) and (val is None):
                try:
                    ...
                    retries += 1
            if val is None:
                raise EurekaMotFixtureError('Fail to read data from particle counter. ')
            return val
...
if __name__ == "__main__":
    ...
    try:
        ...
        for idx in range(0, 100):
            ...
            except EurekaMotFixtureError as e:
                print(e.message)
            finally:
                pass
                # time.sleep(2)
