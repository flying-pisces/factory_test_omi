...
class EurekaMotStation(test_station.TestStation):

    _fixture: test_fixture_eureka_mot.EurekaMotFixture
    _pool: mp.Pool
    ...
    def auto_find_com_ports(self):
        ...
        cfg = 'station_config_eureka_mot.json'
        station_config = {
            'FixtureCom': 'Fixture',
        ...
        if not self._station_config.FIXTURE_SIM and len(port_err_message) > 0:
            raise EurekaMotStationError(f'Fail to find ports for fixture {";".join(port_err_message)}')

    def __init__(self, station_config, operator_interface):
        test_station.TestStation.__init__(self, station_config, operator_interface)
        self._fixture = None
        self._fixture = test_fixture_eureka_mot.EurekaMotFixture(station_config, operator_interface)
    ...
    def _do_test(self, serial_number, test_log):
            ...
            for pos_item in self._station_config.TEST_ITEM_POS:
                ...
                for pattern_name in item_patterns:
                    ...
                    if is_query_temp:
                        ...
                        dut_temp = self._fixture.query_temp(test_fixture_eureka_mot.QueryTempParts.UUTNearBy)
        ...
        except test_equipment_eureka_mot.EurekaMotEquipmentError as e:
            try:
    ...
    def _query_dual_start(self):
        ...
        self._operator_interface.print_to_console("Testing Unit %s\n" % serial_number)
        self._the_unit = EurekaDut(serial_number, self._station_config, self._operator_interface)
    ...
    def data_export(self, serial_number, capture_path, test_log):
        ...
        for pos_item in self._station_config.TEST_ITEM_POS:
            ...
            for pattern_name in item_patterns:
                ...
                file_x = EurekaMotStation.get_filenames_in_folder(capture_path, r'{0}_.*_X_float\.bin'.format(pre_file_name))
                file_y = EurekaMotStation.get_filenames_in_folder(capture_path, r'{0}_.*_Y_float\.bin'.format(pre_file_name))
                ...
                for pri_k in primary:
                    ...
                    try:
                        ...
                        self._pool.apply_async(
                            EurekaMotStation.distortion_centroid_parametric_export_ex_parallel, (
        ...
        for pos_item in self._station_config.TEST_ITEM_POS:
            ...
            if len(file_x) != 0 and len(file_y) == len(file_x) and len(file_z) == len(file_x):
                try:
                    ...
                    self._pool.apply_async(
                        EurekaMotStation.normal_pattern_parametric_export_ex_parallel,
        ...
        for pos_item in self._station_config.TEST_ITEM_POS:
            ...
            for pattern_name, __, __ in item_patterns:
                if pattern_name not in self._station_config.ANALYSIS_GRP_COLOR_PATTERN_EX.keys()\
                    ...
                    continue
                ref_pattern = self._station_config.ANALYSIS_GRP_COLOR_PATTERN_EX[pattern_name]
                file_ref = EurekaMotStation.get_filenames_in_folder(
                ...
                if len(file_x) != 0 and len(file_y) == len(file_x) and len(file_z) == len(file_x) and len(file_ref) > 0:
                    try:
                        ...
                        self._pool.apply_async(
                            EurekaMotStation.grade_a_pattern_parametric_export_ex_parallel,
                            (pos_name, pattern_name, opt),
    ...
    def extract_basic_info_for_pattern(self, capture_path, pattern_name, pos_name, test_log):
        pre_file_name = '{0}_{1}'.format(pos_name, pattern_name)
        json_k = EurekaMotStation.get_filenames_in_folder(
        ...
        file_x = EurekaMotStation.get_filenames_in_folder(capture_path,
                                                            r'{0}_.*_X_float\.bin'.format(pre_file_name))
        file_y = EurekaMotStation.get_filenames_in_folder(capture_path,
